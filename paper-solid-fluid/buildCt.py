...
#--------------------------------------Screen question-----------------------------
gxgx     = raw_input("Do you want to build <gx(t)gx>? (y/n):")
...
#Build the matrix of correlations C when the diagonal does have to be built (<gx(t)gx> and <Sxz(t)Sxz>).
...
#Build the matrix of correlations C when the diagonal does not have to be built (<Sxz(t)Fx>, <Fx(t)Sxz> and <Fx(t)Fx>).
...
if gxgx=='y':
    ...
    np.savetxt('corr-gxgx-B1-t0'   , reshapeBlock_vm(corr_gxgx_B1_avg[0,:]))
    np.savetxt('corr-gxgx-B1-t0.2' , reshapeBlock_vm(corr_gxgx_B1_avg[49,:]))
    np.savetxt('corr-gxgx-B2-t0'   , reshapeBlock_vm(corr_gxgx_B2_avg[0,:]))
    np.savetxt('corr-gxgx-B2-t0.2' , reshapeBlock_vm(corr_gxgx_B2_avg[49,:]))
    ...
    np.savetxt('corr-gxgx'     , C)
    np.savetxt('corr-gxgx-t0'  , reshape_vm(C[0,:]))
    np.savetxt('corr-gxgx-t0.2', reshape_vm(C[49,:]))

#--------------------------------------------------<Sxz(t)Fx>, <Fx(t)Sxz> and <Fx(t)Fx>---------------------------------------

if SxzFx == 'y':
    block1 = np.loadtxt('corr-SxzFx-sf-block1.dat')
    block2 = np.loadtxt('corr-SxzFx-sf-block2.dat')
...
#---------------Average blocks---------------
    ...
    corr_SxzFx_B1_avg  = np.zeros((nSteps, nNodesBlock**2))
    corr_SxzFz_B2_avg  = np.zeros((nSteps, nNodesBlock**2))
    ...
    np.savetxt('corr-SxzSxz-B1-t0', reshapeBlock_vm(corr_SxzSxz_B1_avg[0,:]))
    np.savetxt('corr-SxzSxz-B2-t0', reshapeBlock_vm(corr_SxzSxz_B2_avg[0,:]))
    np.savetxt('corr-SxzFx-B1-t0' , reshapeBlock_vm(corr_SxzFx_B1_avg[0,:]))
    np.savetxt('corr-SxzFx-B2-t0' , reshapeBlock_vm(corr_SxzFx_B2_avg[0,:]))
    np.savetxt('corr-FxSxz-B1-t0' , reshapeBlock_vm(corr_FxSxz_B1_avg[0,:]))
    np.savetxt('corr-FxSxz-B2-t0' , reshapeBlock_vm(corr_FxSxz_B2_avg[0,:]))
    np.savetxt('corr-FxFx-B1-t0'  , reshapeBlock_vm(corr_FxFx_B1_avg[0,:]))
    np.savetxt('corr-FxFx-B2-t0'  , reshapeBlock_vm(corr_FxFx_B2_avg[0,:]))
...
#---------------Build matrices of correlations---------------
    ...
    for t in range(nSteps):
        ...
        Davg, D1avg = avgDiag(B1)
        C[t,:] = reshape_mv(buildC(Z,Davg,D1avg))
    np.savetxt('corr-SxzSxz', C)
    np.savetxt('corr-SxzSxz-t0'  , reshape_vm(C[0,:]))
    np.savetxt('corr-SxzSxz-t0.2', reshape_vm(C[49,:]))
    ...
    np.savetxt('corr-SxzFx', C1)
    np.savetxt('corr-FxSxz', C2)
    np.savetxt('corr-FxFx' , C3)
    np.savetxt('corr-SxzFx-t0'  , reshape_vm(C1[0,:]))
    np.savetxt('corr-FxSxz-t0'  , reshape_vm(C2[0,:]))
    np.savetxt('corr-FxFx-t0'   , reshape_vm(C3[0,:]))
    np.savetxt('corr-SxzFx-t0.2', reshape_vm(C1[49,:]))
    np.savetxt('corr-FxSxz-t0.2', reshape_vm(C2[49,:]))
    np.savetxt('corr-FxFx-t0.2' , reshape_vm(C3[49,:]))
