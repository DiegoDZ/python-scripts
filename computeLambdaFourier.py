...
#-------------------------------------------Input---------------------------------
Ct     = np.loadtxt('Ct_2e3steps-gxTh')                                           #matrix of correlations
...
if space == 'r':
    ...
    for i in np.arange(1, nSteps-1, 1):
        Ctforward  = reshape_vm(Ct[i+1,:])
        Ctbackward = reshape_vm(Ct[i-1,:])
        Ctdev[i]   = reshape_mv((Ctforward - Ctbackward) / (2 * dt))
    for i in np.arange(0,nSteps,1):
        Ctinv[i] = reshape_mv(linalg.pinv(reshape_vm(Ct[i]), rcond = tol))
    #Average over all anti-diagonals for different times
    for t in np.arange(0.1,0.21,0.01):
        Ct      = avgDiag(reshape_vm(Ct[row(t),:]))
        Ctdev   = avgDiag(reshape_vm(Ctdev[row(t),:]))
        Ctinv   = avgDiag(reshape_vm(Ctinv[row(t),:]))
        Lambda  = avgDiag(-reshape_vm(Ctdev[row(t),:]).dot(reshape_vm(Ctinv[row(t),:])))
        kinVisc = avgDiag(-reshape_vm(Ctdev[row(t),:]).dot(reshape_vm(Ctinv[row(t),:])).dot(linalg.pinv(laplacian, rcond=tol)))
        np.savetxt('Ct_2e3steps-gxTh-t'+str(t), Ct)
        np.savetxt('Ctdev_2e3steps-gxTh-t'+str(t), Ctdev)
        np.savetxt('Ctinv_2e3steps-gxTh-t'+str(t), Ctinv)
        np.savetxt('Lambda_2e3steps-gxTh-t'+str(t), Lambda)
        np.savetxt('kinVisc_2e3steps-gxTh-t'+str(t), kinVisc)
...
if space == 'f':
    ...
    for i in range(nSteps):
        CtFourier[i,:]    = reshape_mv(Einv.dot(reshape_vm(Ct[i,:])).dot(E))
    ...
    for t in np.arange(0.1,0.21,0.01):
        ...
        np.savetxt('CtFourier_2e3steps-gxTh-t'+str(t), Ct)
        np.savetxt('CtdevFourier_2e3steps-gxTh-t'+str(t), Ctdev[1:nNodes])
        np.savetxt('CtinvFourier_2e3steps-gxTh-t'+str(t), Ctinv[1:nNodes])
        np.savetxt('LambdaFourier_2e3steps-gxTh-t'+str(t), Lambda[1:nNodes])
        np.savetxt('kinViscFourier_2e3steps-gxTh-t'+str(t), kinVisc)
